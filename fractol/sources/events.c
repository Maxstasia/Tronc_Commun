...
/*   Updated: 2025/01/06 16:58:00 by mstasiak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_scroll(int button, int x, int y, t_data *data)
{
	...
	// Zoom avant (bouton 4) ou arrière (bouton 5)
	if (button == 4)
		zoom_factor = 0.9; // Réduit les limites
	else if (button == 5)
		zoom_factor = 1.1; // Agrandit les limites
	else
		return (0);
	...
	// Reafficher la fractale avec un meilleur calcul de la zone zoomée
	render(data);
	return (0);
}
