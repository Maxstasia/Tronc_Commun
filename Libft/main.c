...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/15 09:56:44 by mstasiak          #+#    #+#             */
/*   Updated: 2024/11/15 16:06:56 by mstasiak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_ft_calloc(void)
{
	...
	// Les deux doivent être NULL ou non NULL (comportement conforme à la norme)
	if ((arr2 == NULL && expected2 != NULL) || (arr2 != NULL && expected2 == NULL)) {
		printf("Test 2 failed: Expected %p, Got %p\n", expected2, arr2);
		test_failed = 1;
	}

// Libérer les pointeurs si nécessaire
if (arr2 != NULL) free(arr2);
if (expected2 != NULL) free(expected2);
...
void test_ft_memcpy(void)
{
	...
	// Test 1 : Copie partielle (5 premiers caractères)
	char dest1[10] = "123456789";
	char src1[10] = "abcdefghi";
	ft_memcpy(dest1, src1, 5);
	if (strncmp(dest1, "abcde6789", 9) != 0) { // Utilisation de strncmp pour limiter les comparaisons
		printf("Test 1 failed: Expected abcde6789, Got %s\n", dest1);
		test_failed = 1;
	}

	// Test 2 : Source plus courte que la destination
	char dest2[10] = "123456789";
	char src2[6] = "abcde";
	ft_memcpy(dest2, src2, 5); // Copie exactement la taille de src2
	if (strncmp(dest2, "abcde6789", 9) != 0) {
		printf("Test 2 failed: Expected abcde6789, Got %s\n", dest2);
		test_failed = 1;
	}

	// Test 3 : Source identique à la destination
	char dest3[10] = "abcdefghi";
	char src3[10] = "abcdefghi";
	ft_memcpy(dest3, src3, 9); // Copie toute la chaîne
	if (strncmp(dest3, "abcdefghi", 9) != 0) {
		printf("Test 3 failed: Expected abcdefghi, Got %s\n", dest3);
		test_failed = 1;
	}

	// Test 4 : Copie de 0 octet
	char dest4[10] = "123456789";
	char src4[10] = "abcdefghi";
	ft_memcpy(dest4, src4, 0); // Ne devrait rien copier
	if (strncmp(dest4, "123456789", 9) != 0) {
		printf("Test 4 failed: Expected 123456789, Got %s\n", dest4);
		test_failed = 1;
	}

	// Test 5 : Taille de copie supérieure à la source (comportement indéfini évité)
	char dest5[15] = "123456789";
	char src5[5] = "abcd";
	ft_memcpy(dest5, src5, 4); // Copie seulement la taille correcte de src5
	if (strncmp(dest5, "abcd56789", 9) != 0) {
...
void test_ft_memmove(void)
{
	...
	// Test 1 : Test avec un déplacement dans une zone mémoire qui se chevauche (vers l'avant)
	char dest1[] = "123456789";
	char expected1[] = "123456789";
	ft_memmove(dest1 + 2, dest1, 5);
	memmove(expected1 + 2, expected1, 5);
	...
	char dest2[13] = "abcdefghijkl";
	char expected2[] = "abcdefghijkl";
	ft_memmove(dest2 + 6, dest2, 6);
	memmove(expected2 + 6, expected2, 6);
	if (strcmp(dest2, expected2) != 0) {
		printf("Test 2 failed: Expected \"%s\", Got \"%s\"\n", expected2, dest2);
		test_failed = 1;
	}

	// Test 3 : Test avec une zone mémoire source et destination distincte
	char dest3[6] = "hello";
	char src3[6] = "world";
	char expected3[] = "hello";
	ft_memmove(dest3, src3, 5);
	memmove(expected3, src3, 5);
	if (strcmp(dest3, expected3) != 0) {
		printf("Test 3 failed: Expected \"%s\", Got \"%s\"\n", expected3, dest3);
		test_failed = 1;
	}

	// Test 4 : Test avec un déplacement de 0 octet (ne devrait rien changer)
	char dest4[] = "abcdef";
	char expected4[] = "abcdef";
	ft_memmove(dest4, dest4, 0);
	memmove(expected4, expected4, 0);
	...
	memmove(expected5 + 4, expected5, 4);
...
void test_ft_memset(void)
{
	...
	ft_memset(str1, 'x', 5);
	if (memcmp(str1, expected1, 10) != 0) {
		printf("Test 1 failed: Expected: %s, Got: %s\n", expected1, str1);
		test_failed = 1;
	}

	// Test 2 : Remplir une chaîne vide
	char str2[10] = {0}; // Initialise tous les caractères à '\0'
	char expected2[10] = "xxxxxxx";
	ft_memset(str2, 'x', 7);
	if (memcmp(str2, expected2, 7) != 0) { // Comparer uniquement les 7 premiers caractères
		printf("Test 2 failed: Expected: %s, Got: %s\n", expected2, str2);
		test_failed = 1;
	}
	...
	char expected4[10] = "zzzzzzzzz"; // Tous les caractères deviennent 'z'
	ft_memset(str4, 'z', 9);
	if (memcmp(str4, expected4, 9) != 0) {
		printf("Test 4 failed: Expected: %s, Got: %s\n", expected4, str4);
		test_failed = 1;
	}
...
/* void test_ft_putchar_fd(void)
{
    ...
    if (fp == NULL) {
        printf("Error: Failed to redirect stdout to /tmp/test_output.txt\n");
        return;
    }
    ft_putchar_fd('A', 1);
    fclose(fp);
    ...
    fp = freopen("/tmp/test_output.txt", "w", stderr);
    if (fp == NULL) {
        printf("Error: Failed to redirect stderr to /tmp/test_output.txt\n");
        return;
    }
    ft_putchar_fd('Z', 2);
    fclose(fp);
...
void test_ft_putendl_fd(void)
{
	...
	if (!temp_file) {
		printf("Test 1 failed: Unable to create temporary file\n");
		return;
	}
...
{
    *c += i; // Modification basée sur l'indice
}
...
void test_ft_striteri(void) {
    ...
    if (strcmp(str4, "XYZ[\\") != 0) { // Modification attendue avec ASCII
    ...
    if (strcmp(str5, "1357") != 0) { // Ajustement en fonction de la logique ASCII
...
char modify_char_2(unsigned int i, char c) {
	return c + i;
}
...
void test_ft_strmapi(void)
{
	...
	char *str1 = "abcd";
	char *result1 = ft_strmapi(str1, modify_char_2);
	...
	// Test 2 : Test avec une chaîne vide ""
	char *str2 = "";
	char *result2 = ft_strmapi(str2, modify_char_2);
	...
	char *str3 = "!@#$";
	char *result3 = ft_strmapi(str3, modify_char_2);
